...
impl PromptSection for WorkspaceSection {
    ...
    fn build(&self, ctx: &PromptContext<'_>) -> Result<String> {
        // No hardcoded path: tools resolve against the *action directory*, not
        // `workspace_dir` (printing that caused write→read mismatches). `shell`
        // is named only to agents that can call it — an uncallable name costs a turn.
        let mut out = String::from(
            "## Workspace\n\n\
             Every file tool resolves a relative path against your working directory. Create \
             files there and read them back from the same place, using the relative path. \
             Writes and reads outside your granted \
        ...
        if ctx.tools.iter().any(|t| t.name == "shell")
            && (ctx.visible_tool_names.is_empty() || ctx.visible_tool_names.contains("shell"))
        {
            out.push_str("`shell` runs in that same directory; `pwd` confirms where it is.\n\n");
        }
