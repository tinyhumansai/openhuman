...
#[path = "memory_fast_path.rs"]
mod memory_fast_path;
use memory_fast_path::*;
...
/// 2. Asks the turn's explicit root-turn dispatch guard whether a delegation
///    can still succeed, and refuses before spending anything if it cannot.
/// 3. Reads the parent's explicit [`ParentExecutionContext`] from its run carrier.
/// 4. Dispatches to `run_typed_mode`.
///
/// On success returns a [`SubagentRunOutcome`] whose `output` is the final
/// assistant text. Failures are suitable for stringifying into a tool result.
