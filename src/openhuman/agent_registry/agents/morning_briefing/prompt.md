...
2. **Frame the window.** Right after the greeting, add one short intro line that states the period this briefing covers in plain words — e.g. "Here's your rundown from the last 24 hours" or "Since yesterday morning…". Ground it on the `Current Date & Time:` line; don't invent a window you didn't actually pull data for.
