...
async fn handle_set_action_dir_persists_valid_path_and_updates_config() {
    ...
    let _ = crate::openhuman::config::ops::load_and_apply_autonomy_settings(
        crate::openhuman::config::ops::AutonomySettingsPatch {
            forbidden_paths: Some(vec![]),
            ..Default::default()
        },
    )
    .await
    .expect("clear forbidden_paths");

    let mut params = serde_json::Map::new();
    params.insert(
        "path".into(),
        serde_json::Value::String(new_action_dir.display().to_string()),
    );

    let out = super::handle_set_action_dir(params)
        .await
        .expect("set_action_dir must succeed for a fresh valid path");
    let inner = out.get("result").unwrap_or(&out);
    let returned = inner
        .get("action_dir")
        .and_then(|v| v.as_str())
    ...
    assert_eq!(
        returned,
        new_action_dir.display().to_string(),
        "the handler must echo back the persisted path"
    );
    assert!(
        new_action_dir.exists(),
        "the handler must create the directory if it doesn't exist yet"
    );
...
async fn handle_set_action_dir_rejects_when_openhuman_action_dir_env_is_set() {
    ...
    let env_value = tmp.path().join("env-forced");

    unsafe {
        std::env::set_var("OPENHUMAN_WORKSPACE", tmp.path());
        std::env::set_var("OPENHUMAN_ACTION_DIR", &env_value);
    }

    let mut params = serde_json::Map::new();
    params.insert(
        "path".into(),
        serde_json::Value::String(attempted.display().to_string()),
    );

    let err = super::handle_set_action_dir(params)
        .await
        .expect_err("handler must reject when OPENHUMAN_ACTION_DIR is set");
    assert!(
        err.contains("OPENHUMAN_ACTION_DIR"),
        "error must mention the env var so callers know what to do; got: {err}"
    );
...
async fn handle_set_action_dir_rejects_path_under_workspace_dir() {
    ...
    let inside = cfg.workspace_dir.join("subdir-3240");

    let mut params = serde_json::Map::new();
    params.insert(
        "path".into(),
        serde_json::Value::String(inside.display().to_string()),
    );

    let err = super::handle_set_action_dir(params)
        .await
        .expect_err("handler must reject paths inside workspace_dir");
    assert!(
        err.to_ascii_lowercase().contains("workspace"),
        "error must mention workspace_dir overlap; got: {err}"
    );
...
#[tokio::test]
async fn handle_set_action_dir_rejects_empty_path() {
    let _g = TEST_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    let tmp = tempfile::tempdir().unwrap();
    unsafe {
        std::env::set_var("OPENHUMAN_WORKSPACE", tmp.path());
    ...
    let mut params = serde_json::Map::new();
    params.insert("path".into(), serde_json::Value::String("   ".into()));

    let err = super::handle_set_action_dir(params)
        .await
        .expect_err("handler must reject empty/whitespace-only paths");
    assert!(
...
async fn handle_get_agent_paths_reports_action_dir_env_override_flag() {
    ...
    std::fs::create_dir_all(&env_value).expect("create env action dir");
    ...
    let out = super::handle_get_agent_paths(serde_json::Map::new())
        ...
        .expect("get_agent_paths handler must succeed");
    let inner = out.get("result").unwrap_or(&out);
    let env_override = inner
        .get("action_dir_env_override")
        .and_then(|v| v.as_bool())
        .expect("action_dir_env_override must be a bool");
    assert!(
        env_override,
        "OPENHUMAN_ACTION_DIR is set → action_dir_env_override must be true"
    );
    ...
    let out = super::handle_get_agent_paths(serde_json::Map::new())
        .await
        .expect("get_agent_paths handler must succeed after env cleared");
    let inner = out.get("result").unwrap_or(&out);
    let env_override = inner
        .get("action_dir_env_override")
        .and_then(|v| v.as_bool())
    ...
    assert!(
        !env_override,
        "OPENHUMAN_ACTION_DIR cleared → action_dir_env_override must be false"
    );
