...
#[cfg(unix)]
#[tokio::test]
async fn validated_symlink_workspace_is_not_recreated_through_trusted_ancestor() {
    let root = tempfile::tempdir().expect("root");
    let trusted = root.path().join("trusted");
    let workspace_target = trusted.join("workspace");
    std::fs::create_dir_all(&workspace_target).expect("workspace target");
    let link_parent = root.path().join("link");
    std::os::unix::fs::symlink(&trusted, &link_parent).expect("trusted link");
    let workspace = link_parent.join("workspace");
    let policy = SecurityPolicy {
        workspace_dir: workspace.clone(),
        action_dir: workspace.clone(),
        workspace_only: true,
    ...
    let resolved = policy
        ...
        .expect("initial validation");
    let parent = resolved.parent().expect("resolved parent");
    std::fs::remove_dir_all(&workspace_target).expect("remove workspace target");

    let error = create_validated_parent_dirs(&policy, parent)
        .await
        .expect_err("removed symlink workspace must not be recreated");
    assert_eq!(error.kind(), std::io::ErrorKind::NotFound);
