...
pub fn upsert_agent_run(config: &Config, upsert: AgentRunUpsert) -> Result<AgentRun> {
    ...
    crate::openhuman::agent::session_db::store::with_connection(config, |conn| {
...
pub fn claim_agent_team_task(
    ...
    let outcome = crate::openhuman::agent::session_db::store::with_connection(config, |conn| {
        init_run_ledger_schema(conn)?;
...
pub fn shutdown_agent_team_member(
    ...
    let result = crate::openhuman::agent::session_db::store::with_connection(config, |conn| {
        init_run_ledger_schema(conn)?;
