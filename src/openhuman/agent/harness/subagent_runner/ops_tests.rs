...
fn max_result_chars_cap_is_enforced() {
    // Verify that `max_result_chars` truncation logic is correct.
    // We test the truncation formula directly since running the full
    // subagent loop requires a provider.
    let cap = 10usize;
    let mut output = "hello world this is long".to_string();
    if output.len() > cap {
        output.truncate(cap);
        output.push_str("\n[...truncated]");
    }
    assert!(output.starts_with("hello worl"));
