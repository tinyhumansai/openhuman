...
fn max_result_chars_cap_is_enforced() {
    ...
    // Simulate the logic in ops.rs (char-safe path).
...
fn max_result_chars_cap_is_char_safe_for_multibyte() {
    ...
    // not panic. "café" has 4 chars but the 'é' is 2 bytes — truncating
    // at byte 4 with the old String::truncate() would panic.
