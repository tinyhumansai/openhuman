...
fn handle_get_settings(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await.map_err(|e| {
            tracing::warn!("[learning.get_settings] config load failed: {e}"); e
        })?;
        let enabled = config.learning.enabled;
        RpcOutcome::new(serde_json::json!({"enabled": enabled}), vec![format!("learning.get_settings: enabled={enabled}")]).into_cli_compatible_json()
    })
}
...
fn handle_update_settings(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        config.save().await.map_err(|e| {
            tracing::warn!("[learning.update_settings] config save failed: {e}"); format!("failed to save learning settings: {e}")
        })?;
        RpcOutcome::new(serde_json::json!({"enabled": enabled}), vec![format!("learning.update_settings: enabled={enabled} (takes effect on new sessions)")]).into_cli_compatible_json()
    })
}
