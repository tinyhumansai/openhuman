...
pub fn learning_schemas(function: &str) -> ControllerSchema {
    match function {
        ...
        "learning_get_settings" => ControllerSchema {
            namespace: "learning", function: "get_settings",
            description: "Read the self-learning master switch (`learning.enabled`).",
            inputs: vec![], outputs: vec![FieldSchema { name: "enabled", ty: TypeSchema::Bool, comment: "Whether capture, scoring, and injection are enabled.", required: true }],
        },
        "learning_update_settings" => ControllerSchema {
            namespace: "learning", function: "update_settings",
            description: "Update and persist the self-learning master switch.",
            inputs: vec![FieldSchema { name: "enabled", ty: TypeSchema::Bool, comment: "Set learning.enabled to this value.", required: true }],
            outputs: vec![FieldSchema { name: "enabled", ty: TypeSchema::Bool, comment: "The persisted learning.enabled value.", required: true }],
        },
        _ => ControllerSchema {
            namespace: "learning",
            function: "unknown",
