...
pub fn ensure_reembed_backfill(config: &crate::openhuman::config::Config) {
    let sig = crate::openhuman::memory_tree::store::tree_active_signature(config);
    let result = crate::openhuman::memory_tree::store::with_connection(config, |conn| {
        Ok(crate::openhuman::memory_tree::store::has_uncovered_reembed_work(conn, &sig)?)
    });
    match result {
        Ok(true) => {
