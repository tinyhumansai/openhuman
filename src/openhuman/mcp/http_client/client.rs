...
/// pipeline (`util::sanitize`) is always applied. The raw fields
...
impl McpRemoteTool {
    ...
    /// `util::sanitize` pipeline (control-char strip, instruction-
    ...
    pub fn display_description(&self) -> Option<String> {
        self.description.as_deref().map(|d| {
            crate::openhuman::util::sanitize::sanitize_for_llm(
                d,
                crate::openhuman::util::sanitize::MAX_DESCRIPTION_BYTES,
            )
        })
    }
    ...
    /// [`crate::openhuman::util::sanitize::MAX_TITLE_BYTES`].
...
/// the user-facing copy is derived separately in `mcp::registry::connections`.
#[derive(Debug, Clone)]
