...
//! similar) lives in [`crate::openhuman::mcp::config_servers`] and
//! [`crate::openhuman::mcp::http_client`] — this module just
...
use crate::openhuman::mcp::http_client::{McpHttpClient, McpRemoteTool};
...
/// [`crate::openhuman::mcp::config_servers::McpTransportClient`] but lives
/// here so `mcp::registry` doesn't have to depend on the static-config
...
impl ActiveClient {
    ...
    async fn call_tool(
        ...
        name: &str,
        arguments: Value,
    ) -> anyhow::Result<crate::openhuman::mcp::http_client::McpServerToolResult> {
        match self {
...
/// `mcp::registry::connections::ConnectedServerOverview` path keeps resolving.
...
fn auth_failure_kind(err: &anyhow::Error, has_credential: bool) -> Option<AuthFailureKind> {
    let unauthorized = err.chain().find_map(|cause| {
        cause.downcast_ref::<crate::openhuman::mcp::http_client::McpUnauthorizedError>()
    })?;
...
async fn connect_inner(config: &Config, server: &InstalledServer) -> anyhow::Result<Vec<McpTool>> {
    ...
    let safe_remote_tools =
        crate::openhuman::mcp::config_servers::apply_safety_filter(&server.server_id, remote_tools);
...
mod tests {
    ...
    use crate::openhuman::mcp::registry::types::ServerStatus;
