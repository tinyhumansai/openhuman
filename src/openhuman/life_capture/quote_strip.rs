...
static ON_DATE_WROTE: Lazy<Regex> = Lazy::new(|| {
    ...
    // including wrapped/multi-line attribution headers from Gmail.
    Regex::new(r"(?ms)^On .{1,200}?\bwrote:\s*$").unwrap()
});
...
mod tests {
    ...
    #[test]
    fn strips_wrapped_on_date_wrote_block_and_below() {
        let input = "Quick reply.\n\nOn Mon, Apr 21, 2026 at 9:14 AM,\nSarah <sarah@x> wrote:\n> earlier text\n";
        assert_eq!(strip_quoted_reply(input), "Quick reply.");
    }
