    ...
    match function {
        ...
        "ingest" => ControllerSchema {
            ...
            inputs: vec![
                ...
                FieldSchema {
                    name: "metadata",
                    ty: TypeSchema::Option(Box::new(TypeSchema::Map(Box::new(TypeSchema::Json)))),
...
fn handle_ingest(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let metadata = read_optional_json_object(&params, "metadata")?
            .map(Value::Object)
...
fn read_optional_json_object(
    ...
    match params.get(key) {
        ...
        Some(other) => Err(format!(
            "invalid '{key}': expected object, got {}",
            type_name(other)
        )),
    }
}
...
mod tests {
    ...
    #[test]
    fn schemas_ingest_metadata_is_json_object() {
        let s = schemas("ingest");
        let metadata = s.inputs.iter().find(|f| f.name == "metadata").unwrap();
        assert_eq!(
            metadata.ty,
            TypeSchema::Option(Box::new(TypeSchema::Map(Box::new(TypeSchema::Json))))
        );
    }

    #[test]
    fn read_optional_json_object_rejects_non_object_metadata() {
        let mut params = Map::new();
        params.insert("metadata".into(), Value::String("{}".into()));
        let err = read_optional_json_object(&params, "metadata").unwrap_err();
