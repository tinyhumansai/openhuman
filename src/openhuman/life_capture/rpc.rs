...
mod tests {
    ...
    #[tokio::test]
    async fn ingest_does_not_upsert_item_when_embedding_fails() {
        ...
        let stats = handle_get_stats(&idx).await.unwrap();
        assert_eq!(stats.value["total_items"], 0);
    }
}
