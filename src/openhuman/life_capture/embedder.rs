...
impl HostedEmbedder {
    pub fn new(base_url: String, api_key: String, model: String) -> Self {
        let http = reqwest::Client::builder()
            ...
            .build()
            .expect("failed to build reqwest client with embedding timeouts");
        let embedding_dim = embedding_dim_for_model(&model);
        Self {
            base_url,
            api_key,
            model,
...
mod tests {
    ...
    #[test]
    fn hosted_embedder_dim_tracks_known_model() {
        let emb = HostedEmbedder::new(
            "http://127.0.0.1:1/v1".into(),
            "test-key".into(),
            "text-embedding-3-large".into(),
        );
        assert_eq!(emb.dim(), 3072);
    }
}
