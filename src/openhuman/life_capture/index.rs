...
impl IndexReader {
    ...
    pub async fn hybrid_search(
        ...
        let mut oversample = (q.k * 3).max(20);
        let max_oversample = (q.k * 50).clamp(oversample, 1_000);
        ...
        loop {
            ...
            let mut out: Vec<Hit> = by_id
                ...
                .map(|(mut hit, kw_rrf, vc_rrf)| {
                    ...
                    hit.score =
                        W_VEC * vc_rrf + W_KW * kw_rrf + W_RECENCY * recency * recency_scale;
                    hit
                })
                .collect();
            out.sort_by(|a, b| {
            ...
            if out.len() >= q.k || oversample >= max_oversample {
                out.truncate(q.k);
                return Ok(out);
            }
