...
pub(crate) struct ToolPolicyEnforcement {
    ...
    /// True when the caller explicitly scoped the visible tool set. Kept for the
    /// turn seam contract even though unknown-tool recovery is now owned by the
    /// tinyagents SDK's valid-tools hint.
