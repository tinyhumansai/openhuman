...
mod tests {
    ...
    #[test]
    fn extract_issue_identifier_finds_workspace_prefix() {
    ...
    fn extract_issue_identifier_none_when_missing() {
        let issue = json!({ "id": "u1", "title": "Without identifier" });
    ...
    #[test]
    fn extract_issue_updated_handles_nested_data() {
    ...
    #[test]
    fn extract_viewer_id_from_top_level_viewer() {
        let data = json!({ "viewer": { "id": "user-uuid-2" } });
        assert_eq!(extract_viewer_id(&data), Some("user-uuid-2".into()));
    }
    ...
    #[test]
    fn extract_pagination_end_cursor_none_when_no_next() {
        // hasNextPage: false should NOT return the cursor, even if it's
        // present — otherwise the caller would loop forever fetching
        // the same end-of-results page.
        let data = json!({
            "data": {
                "issues": {
                    "pageInfo": {
