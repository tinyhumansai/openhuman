...
//!   * Per-connection state (last sync timestamp) lives in an in-memory
//!     `HashMap` keyed by `(toolkit, connection_id)`. This is rebuilt
//!     on restart, which is fine — a missed periodic sync is harmless
...
async fn run_loop() {
    let mut last_sync_at: HashMap<(String, String), Instant> = HashMap::new();
    ...
    loop {
        ticker.tick().await;
        if let Err(e) = run_one_tick(&mut last_sync_at).await {
            tracing::warn!(
                error = %e,
...
pub(crate) async fn run_one_tick(
    last_sync_at: &mut HashMap<(String, String), Instant>,
) -> Result<(), String> {
    ...
    for conn in resp.connections {
        ...
        let due = match last_sync_at.get(&key) {
        ...
        match provider.sync(&ctx, SyncReason::Periodic).await {
            Ok(outcome) => {
                ...
                last_sync_at.insert(key, Instant::now());
                fired += 1;
            }
            Err(e) => {
                tracing::warn!(
