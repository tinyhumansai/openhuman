...
const messages: TranslationMap = {
  ...
  'graphCohesion.intro':
    '각 개체 주변 이웃이 얼마나 긴밀하게 연결되어 있는지를 나타냅니다. 브로커 — 이웃들이 서로 연결되지 않은 개체 — 는 그렇지 않으면 분리될 클러스터들을 이어주는 유일한 연결 고리이며, 빈도 또는 PageRank 정렬로는 이를 파악할 수 없습니다.',
