...
/**
 * Safety cap: BFS from every node is O(V·(V+E)); beyond this many nodes the
 * computation can freeze the browser tab. Callers should pre-check with
 * `isGraphReachFeasible` and show a warning instead of blocking the UI.
 */
export const MAX_REACH_NODES = 5_000;

/** Quick check: will `computeGraphReach` be fast enough to run inline? */
export function isGraphReachFeasible(relations: GraphRelation[]): boolean {
  const seen = new Set<string>();
  for (const r of relations) {
    if (!isRelation(r)) continue;
    seen.add(r.subject);
    seen.add(r.object);
    if (seen.size > MAX_REACH_NODES) return false;
  }
  return true;
}
