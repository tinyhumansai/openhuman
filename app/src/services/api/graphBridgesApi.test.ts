...
import { graphBridgesApi, loadBridges, loadNamespaces } from './graphBridgesApi';
...
describe('graphBridgesApi.loadBridges', () => {
  beforeEach(() => {
  ...
  it('passes the namespace through and returns the engine result for those triples', async () => {
    ...
    const out = await loadBridges('work');
    expect(mockGraphQuery).toHaveBeenCalledWith('work');
    expect(out).toEqual(computeGraphBridges(triples));
    expect(out.articulationCount).toBe(1);
  });

  it('queries all namespaces when none is given', async () => {
    mockGraphQuery.mockResolvedValueOnce([]);
    const out = await loadBridges();
  ...
  it('propagates query errors', async () => {
    ...
    await expect(loadBridges()).rejects.toThrow('graph unavailable');
  });
});

describe('graphBridgesApi.loadNamespaces', () => {
  beforeEach(() => {
...
describe('graphBridgesApi object', () => {
  it('exposes the public surface', () => {
    expect(typeof graphBridgesApi.loadBridges).toBe('function');
    expect(typeof graphBridgesApi.loadNamespaces).toBe('function');
  });
});
