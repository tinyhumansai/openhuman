/**
 * Evidence-Weighted Trust tab (container). Owns load-on-mount and the
 * namespace selector; delegates all rendering to the pure
 * <EvidenceTrustPanel>. Read-only — the result is recomputed from the live
...
import { loadEvidenceTrust, loadNamespaces } from '../../services/api/evidenceTrustApi';
import EvidenceTrustPanel from './EvidenceTrustPanel';

const EvidenceTrustTab = () => {
  const { t } = useT();
  const [result, setResult] = useState<EvidenceTrustResult | null>(null);
  ...
  const load = useCallback(async (ns: string) => {
    ...
    setError(null);
    try {
      const next = await loadEvidenceTrust(ns || undefined);
    ...
    <div className="space-y-4">
      {namespaces.length > 0 && (
        <label className="flex items-center gap-2 text-xs text-stone-600 dark:text-neutral-300">
          {t('evidenceTrust.namespaceLabel')}
          <select
            value={namespace}
            ...
            <option value="">{t('evidenceTrust.namespaceAll')}</option>
            {namespaces.map(ns => (
...
export default EvidenceTrustTab;
