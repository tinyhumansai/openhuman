/**
 * Document Provenance tab (container). Owns load-on-mount and the namespace
 * selector; delegates all rendering to the pure <DocumentProvenancePanel>.
...
import { loadNamespaces, loadProvenance } from '../../services/api/documentProvenanceApi';
import DocumentProvenancePanel from './DocumentProvenancePanel';

const DocumentProvenanceTab = () => {
  const { t } = useT();
  ...
  const load = useCallback(async (ns: string) => {
    ...
    setError(null);
    try {
      const next = await loadProvenance(ns || undefined);
  ...
  useEffect(() => {
    ...
    // the provenance view, so swallow that error specifically.
    loadNamespaces()
      .then(setNamespaces)
      .catch(() => setNamespaces([]));
    void load('');
    ...
    <div className="space-y-4">
      {namespaces.length > 0 && (
        <label className="flex items-center gap-2 text-xs text-stone-600 dark:text-neutral-300">
          {t('documentProvenance.namespaceLabel')}
          <select
            value={namespace}
            ...
            <option value="">{t('documentProvenance.namespaceAll')}</option>
            {namespaces.map(ns => (
...
export default DocumentProvenanceTab;
