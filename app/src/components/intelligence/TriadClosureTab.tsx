/**
 * Triad Closure tab (container). Owns load-on-mount and the namespace
 * selector; delegates all rendering to the pure <TriadClosurePanel>. Read-
 * only — the result is recomputed from the live graph, never persisted.
 */
import { useCallback, useEffect, useRef, useState } from 'react';

import { useT } from '../../lib/i18n/I18nContext';
import type { TriadClosureResult } from '../../lib/memory/triadClosure';
import { loadNamespaces, loadTriadClosure } from '../../services/api/triadClosureApi';
import TriadClosurePanel from './TriadClosurePanel';

const TriadClosureTab = () => {
  const { t } = useT();
  const [result, setResult] = useState<TriadClosureResult | null>(null);
  ...
  useEffect(() => {
    ...
    // the worklist, so swallow that error specifically.
    loadNamespaces()
      .then(setNamespaces)
      .catch(() => setNamespaces([]));
    void load('');
      ...
      {namespaces.length > 0 && (
        <label className="flex items-center gap-2 text-xs text-stone-600 dark:text-neutral-300">
          ...
          <select
            ...
            <option value="">{t('triadClosure.namespaceAll')}</option>
            {namespaces.map(ns => (
