...
export default function IntelligenceSubconsciousTab({
  ...
  const handleSliderCommit = useCallback(() => {
    ...
    if (minutes !== intervalMinutes) void setIntervalMinutes(minutes);
        ...
        <div className="grid grid-cols-3 gap-2">
          {MODE_OPTIONS.map(opt => (
            <button
              ...
              className={`flex flex-col items-center text-center rounded-lg border p-3 transition ${mode === opt.id ? 'border-primary-500 bg-primary-50 dark:bg-primary-500/10' : 'border-line hover:border-primary-300 dark:hover:border-primary-500/40'} ${settingMode ? 'opacity-60 cursor-wait' : ''}`}>
              <span
                className={`inline-block w-3 h-3 rounded-full border-2 mb-1.5 ${mode === opt.id ? 'bg-primary-500 border-primary-500' : 'border-line-strong dark:border-neutral-600'}`}
              />
