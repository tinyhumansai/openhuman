/**
 * Graph Bridges tab (container). Owns load-on-mount and the namespace selector;
 * delegates all rendering to the pure <GraphBridgesPanel>. Read-only — the
 * result is recomputed from the live graph, never persisted.
 */
import { useCallback, useEffect, useRef, useState } from 'react';

import { useT } from '../../lib/i18n/I18nContext';
import type { BridgeResult } from '../../lib/memory/graphBridges';
import { loadBridges, loadNamespaces } from '../../services/api/graphBridgesApi';
import GraphBridgesPanel from './GraphBridgesPanel';

const GraphBridgesTab = () => {
  const { t } = useT();
  ...
  const load = useCallback(async (ns: string) => {
    ...
    setError(null);
    try {
      const next = await loadBridges(ns || undefined);
  ...
  useEffect(() => {
    ...
    // the bridges view, so swallow that error specifically.
    loadNamespaces()
      .then(setNamespaces)
      .catch(() => setNamespaces([]));
    void load('');
      ...
      {namespaces.length > 0 && (
        <label className="flex items-center gap-2 text-xs text-stone-600 dark:text-neutral-300">
          ...
          <select
            ...
            <option value="">{t('graphBridges.namespaceAll')}</option>
            {namespaces.map(ns => (
...
export default GraphBridgesTab;
