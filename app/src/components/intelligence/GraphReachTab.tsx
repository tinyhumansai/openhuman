/**
 * Graph Reach tab (container). Owns load-on-mount and the namespace selector;
 * delegates all rendering to the pure <GraphReachPanel>. Read-only — the result
...
import { loadNamespaces, loadReach } from '../../services/api/graphReachApi';
...
const GraphReachTab = () => {
  ...
  const load = useCallback(async (ns: string) => {
    ...
    setError(null);
    try {
      const next = await loadReach(ns || undefined);
      ...
      {namespaces.length > 0 && (
        <label className="flex items-center gap-2 text-xs text-stone-600 dark:text-neutral-300">
          ...
          <select
            ...
            <option value="">{t('graphReach.namespaceAll')}</option>
            {namespaces.map(ns => (
