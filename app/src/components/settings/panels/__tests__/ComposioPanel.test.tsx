...
describe('ComposioPanel', () => {
  ...
  test('shows a localized fallback when RPC throws an unknown error', async () => {
  ...
  test('localizes the known invalid API key error', async () => {
    hoisted.setApiKey.mockRejectedValue(new Error('Invalid Composio API key. Re-enter your key.'));
    const Panel = await importPanel();
    renderWithProviders(<Panel />);
    await waitFor(() => expect(screen.queryByText('Loading…')).toBeNull());
    ...
    await waitFor(() => expect(screen.getByText('Invalid Composio API key.')).toBeInTheDocument());
  });
