  ...
  useEffect(() => {
    ...
    const fetchConfig = async () => {
      try {
        ...
        if (mounted) {
          const storedUrl = result.config.api_url;
          const storedKey = result.config.api_key;
          console.debug('[CustomModelSection] fetched config:', {
            hasApiUrl: Boolean(storedUrl),
            hasApiKey: Boolean(storedKey),
          });
          setApiUrl(String(storedUrl || ''));
        ...
        if (mounted) {
          console.error('[CustomModelSection] failed to load config:', err);
